...
class LinearSpaceItemDecoration(
    ...
    private val conditionProvider: ConditionProvider? = null,
    ...
    private fun makeLinearOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State): Rect =
        outRect.apply {
            ...
            if (position != RecyclerView.NO_POSITION) {
                when (orientation) {
                    VERTICAL -> {
                        if (position != 0 || showFirstHorizontalDivider) {
                            if (conditionProvider?.shouldDrawDivider(position) != false) {
                                top = space
                            }
                        }
                        if (position == state.itemCount - 1 && showLastHorizontalDivider) {
                            if (conditionProvider?.shouldDrawDivider(position) != false) {
                                bottom = space
                            }
                        }
